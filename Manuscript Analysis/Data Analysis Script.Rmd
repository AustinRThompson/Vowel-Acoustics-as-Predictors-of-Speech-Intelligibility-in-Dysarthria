...
geom.text.size <- 2.5
    VSDplot <- ggplot(data = plotData,
      ...
      theme(plot.title = element_text(hjust = 0.5),
            aspect.ratio = 1) +
      geom_polygon(data = nconvex_25, alpha = lineAlpha, color = lineColor, size = 1.5, fill = NA, linetype = 1) +
      geom_polygon(data = nconvex_75, alpha = lineAlpha, color = lineColor, size = 1.5, fill = NA, linetype = 2) +
      geom_labelcurve(data = data.frame(x = 10.382452, xend = 8.313515, y = 2.874967, yend = 4.127919, Density = 1), 
                 ...
                 fill = "white",
                 rich = TRUE) +
        geom_labelcurve(data = data.frame(x = 14.92512, xend = 12.04660, y = 3.95, yend = 6, Density = 1), 
                 aes(x, y, xend = xend, yend = yend),
                 hjust = 0.3, 
                 vjust = 0,
                 curvature = 0,
