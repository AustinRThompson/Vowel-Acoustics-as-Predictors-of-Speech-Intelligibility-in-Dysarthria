...
AcousticDataLong <- AcousticDataLong %>%
  ...
  dplyr::mutate(Acoustic_Measure = case_when(
    Acoustic_Measure == "Hull_bVSD_25" ~ "VSD 25",
    Acoustic_Measure == "Hull_bVSD_75" ~ "VSD 75",
    Acoustic_Measure == "Hull_b" ~ "Vowel Space Hull",
    Acoustic_Measure == "VSA_b" ~ "Vowel Space Area",
    Acoustic_Measure == "vowel_ED_b" ~ "Corner Dispersion"),
    Acoustic_Measure = as.factor(Acoustic_Measure),
                Acoustic_Measure = fct_relevel(Acoustic_Measure, c("VSD 25", 
                                                                   "VSD 75",
                                                                   "Vowel Space Hull",
                                                                   "Vowel Space Area",
                                                                   "Corner Dispersion"))) %>%
...
AcousticDataLong %>%
  ...
  scale_color_manual(values = myPal) +
  scale_shape_manual(values = myShapes) +
  labs(x = "Vowel Acoustic Measures", y = "Intelligibility") +
  theme_bw()

ggsave("Plots/Etiology_Effects_Intelligibility_plot.png", plot = last_plot(), height = 5, width = 10, units = "in")
