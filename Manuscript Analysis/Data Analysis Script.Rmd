...
CorrMatrix <- AcousticData %>%
  dplyr::select(VSA_b, vowel_ED_b, Hull_b, Hull_bVSD_25, Hull_bVSD_50, Hull_bVSD_75, autoVSA, VAS, transAcc) %>%
  as.matrix() %>%
  Hmisc::rcorr()
...
OT_Model3 <- lm(transAcc ~ Hull_bVSD_75 + Hull_b, data = AcousticData)

## Model 3 Assumption Check

performance::check_model(OT_Model3)
performance::check_collinearity(OT_Model3)
...
OT_Model4 <- lm(transAcc ~ Hull_bVSD_75 + autoVSA, data = AcousticData)
...
VAS_Model3 <- lm(VAS ~ Hull_bVSD_75 + Hull_b, data = AcousticData)

## Model 3 Assumption Check

performance::check_model(VAS_Model3)
performance::check_collinearity(VAS_Model3)
...
VAS_Model4 <- lm(VAS ~ Hull_bVSD_75 + autoVSA, data = AcousticData)
...
scatter <- VSA + autoVSA + disp + Hull + vsd25 + vsd75 +
  patchwork::plot_layout(guides = 'collect',
